...
@Component({
  selector: 'app-flipcardstart',
  standalone: true,
  imports: [CommonModule, HeaderComponent, SideBarComponent, 
    HeadergameComponent, RouterModule  ],
  templateUrl: './flipcardstart.component.html',
  styleUrls: ['./flipcardstart.component.css']
})
export class FlipcardstartComponent implements OnInit {
  wordSetId!: number;
  vocabCount = 0;
  selectedMode: { rows: number, cols: number } | null = null;
  wordSetTitle: string = '';
    ...
    this.wordsetService.getWordSetById(this.wordSetId).subscribe({
      next: (wordSet) => {
        if (wordSet && wordSet.words) {
          this.vocabCount = wordSet.words.length;
          this.wordSetTitle = wordSet.title;
          console.log('Số từ hiện có:', this.vocabCount);
        } else {
  ...
  selectMode(rows: number, cols: number) {
    this.selectedMode = { rows, cols };
  }
